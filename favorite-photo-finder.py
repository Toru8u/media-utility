#!/usr/bin/env python3
"""
Favorite Photo Finder (pure Python/XMP XML, rekursiv!)
Findet und kopiert JPGs mit XMP Sternbewertung ins Zielverzeichnis – rekursiv!
"""
import os
import sys
import shutil
...
def extract_xmp(file_path):
    """Extract XMP XML block from JPEG binary."""
...
def get_xmp_rating(xmp_str):
    """Extract star rating from XMP XML (Adobe/Windows)."""
    if not xmp_str:
        return 0
    try:
...
def get_xmp_keywords(xmp_str):
    """Extract keywords (dc:subject, xmp:Keywords) from XMP."""
    keywords = set()
    if not xmp_str:
        return keywords
    try:
...
def find_jpeg_files(src_dir):
    """Recursively find all JPEGs in src_dir."""
    files = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Find JPGs with XMP star rating (pure Python/XML, recursive)",
